...
- 根据已保存的长期用户画像生成随当前皮肤变化的四维角色观察报告，不重复分析聊天记录
...
话题、称号、金句、聊天质量、长期画像、自然语言查询、群漫画分镜和用户画像漫画的提示词均由插件内置，不在配置界面提供编辑项。升级插件后会自动采用新版提示词；ChatLuna 预设仍可用于调整模型的人格与表达倾向。
...
启用“漫画服务”和“启用用户画像漫画”后，可使用 `用户画像.漫画` 或空格写法 `用户画像 漫画`。漫画固定包含“总体概览、性格特质、兴趣爱好、沟通风格”四个分镜，每格都必须对应回已有画像，不会重新分析聊天记录，也不会在没有已保存画像时调用生图接口。

生成时，插件会先用当前皮肤渲染普通用户画像，并把它作为内容与主题参考图；配置的角色参考图则作为主持角色的外观依据。两类参考图在提示词中严格区分，避免把被分析用户的头像与主持角色混为一体。13 个可选皮肤按 5 个基础视觉族生成不同的报告式漫画，并为简洁、亚托莉、碧蓝档案、初音未来、复古未来、终端、新艺术和节日皮肤追加专属视觉方向，因此不会全部变成手账风格。

分镜模型在同一次调用中完成“四维提炼 → 逐格映射”，随后调用已有生图服务。事实依据用于设计场景，不会要求把大段原始记录画进气泡。画像缺少至少三个有效维度时提示数据不足。

无画像时会提示先使用“用户画像”生成普通画像，不会自动执行画像分析，也不会请求生图接口。画像过期也直接使用已保存版本；需要更新时先执行 `用户画像 -f`。

两个漫画入口共用接口、密钥、模型、超时、角色方案、并发限制与每群冷却。用户画像漫画使用竖版画布，群漫画继续使用原有横版尺寸。`comic.userEnabled` 默认关闭；四维报告与各主题的生成提示词由插件内置，升级后自动使用最新版本，不在配置界面提供覆盖项。角色方案只控制主持角色外观，用户的性格与行为始终以已保存画像为准。没有新增漫画历史表。
...
| `群分析.用户画像 -f`                              | 生成自己的普通画像；`-f` 强制刷新。                                                          |
| `群漫画 [-d 天数]`                                | 从最近 1–7 天的群聊话题生成漫画。                                                            |
| `用户画像`                                        | 生成并查看自己的普通用户画像。根命令不带用户参数，以便正确识别空格子命令。                   |
| `用户画像 漫画 [@用户]` / `用户画像.漫画 [@用户]` | 不传参数时生成自己的漫画版；管理员可通过 @用户生成其他人的漫画版。没有已保存画像时不会生图。 |
